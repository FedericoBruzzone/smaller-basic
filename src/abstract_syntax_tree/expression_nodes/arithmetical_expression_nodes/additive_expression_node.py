...
class AdditiveExpressionNode(ArithmeticalExpressionNode):
    ...
    def __init__(
        ...
        if right_expression_node and not isinstance(right_expression_node, MultiplicativeExpressionNode):
            raise ValueError(f"Right expression node must be of type MultiplicativeExpressionNode. Got: {type(right_expression_node)}")
